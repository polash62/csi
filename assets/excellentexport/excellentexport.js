...
var ExcellentExport = (function() {
    ...
    var ee = {
        ...
        excel: function(anchor, table, name,user_id, action) {
            ...
            htmls += '<h4>' + $('#report-name').html() + '</h4>';

            htmls += '<h4>' + $('#prodName').html().trim() + '&nbsp;&nbsp;' + $('#divName').html().trim() + '&nbsp;&nbsp;' + $('#regName').html().trim() + '&nbsp;&nbsp;' + $('#areaName').html().trim() + '&nbsp;&nbsp;' + $('#brName').html() + $('#VoCode').html().trim() + '&nbsp;&nbsp;' + $('#memberNo').html() + '</h4>';

            var t = $('#'+table).clone();
            t.appendTo(document.body);
            t.find('td').not(':visible').remove();
            t.find('th').not(':visible').remove();

            htmls += t.html();
            t.remove();

            var ctx = {worksheet: name || 'Worksheet', table: htmls};
